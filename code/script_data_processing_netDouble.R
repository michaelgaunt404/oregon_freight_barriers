...
inflow_links = inflow_links %>%
  st_drop_geometry()  %>%
  select(!ends_with(c("ENO", "_NO", "_FROMNO"))) %>%
  mutate(sum_tv = rowSums(select(., starts_with(c("T_", "V_"))))
         ,sum_r = rowSums(select(., starts_with(c("R_"))))) %>%
  select(starts_with("sum_")) %>%
  bind_cols(inflow_links, .)
...
cities = tigris::urban_areas(year = 2020) %>%
  st_transform(4326) %>%
  st_filter(states_or)
...
###SUB: uni-bimodal link exploration--------------------------------------------
#confirming that I84 and I5 are made of unimodal links
# buffer = data.frame(long = 45.636315, lat =  -120.995548) %>%
#   st_as_sf(coords = c("lat", "long"), crs = st_crs(4326)) %>%
#   quick_buffer(radius = 20*1600)
#
# buffer %>%  quick_leaflet(polys = T)
#
# index_unimodal_tv0 = inflow_links %>%
#   st_drop_geometry() %>%
#   select(!ends_with(c("ENO", "_NO", "_FROMNO"))) %>%
#   filter( rowSums(select(., starts_with(c("T_", "V_")))) == 0
#           & rowSums(select(., starts_with(c("R_")))) != 0) %>%
#   pull(NO)
#
# index_unimodal_r0 = inflow_links %>%
#   st_drop_geometry() %>%
#   select(!ends_with(c("ENO", "_NO", "_FROMNO"))) %>%
#   filter(rowSums(select(., starts_with(c("T_", "V_")))) != 0
#          & rowSums(select(., starts_with(c("R_")))) == 0) %>%
...
#   pull(NO)
#
# index_links_empty = inflow_links %>%
#   st_drop_geometry() %>%
#   select(!ends_with(c("ENO", "_NO", "_FROMNO"))) %>%
#   filter(rowSums(select(., starts_with(c("T_", "V_", "R_")))) == 0 ) %>%
#   pull(NO)
#
# inflow_links %>%
#   # filter(NO %not_in% index_links_empty
#   #        ,NO %not_in% index_bimodal) %>%
#   filter(NO %not_in% index_links_empty
#                 ,NO %in% index_bimodal) %>%
#   leaflet() %>%
#   addTiles() %>%
#   addPolylines(label = ~str_glue("{NO} ---- {sum_tv} ---- {sum_r}")
...
temp = inflow_links %>%
  ...
  select(!ends_with(c("ENO", "_NO", "_FROMNO", "_r", "_tv"))) %>%
...
leaflet(tmp) %>%
  ...
  addPolylines(color = ~binpal(corridor)
               # ,weight = tmp$width
               ,opacity = 1
               ,label = ~str_glue("{corridor} ----- {NO} ----- {value}")
               ,labelOptions = labelOptions(noHide = F, textOnly = F)
...
#section makes maps using driver data.frame
#IMPORTANT::::::
#this section a be performed using both uni- and bi-directional links
#---> it is agnostic to corridors/facilities having single or double links
temp_tbl = temp_sp_cor %>%
  st_drop_geometry() %>%
  rename(count = "value")

yolo = c("T_", "Ton", "V_", "Val") %>%
  map(~
        temp_tbl %>%
...
#this section performs the final processing for mapping
#most importantly is doubles the values for the I5 and I84 corridors
#then it applies all of the other mutatings required for the maps

temp_sp_cor_dbl = temp_sp_cor %>%
  mutate(value = case_when(corridor %in% c('buffer_84', 'buffer_i5') ~ 2*value
                           ,T~value)) %>%
  group_by(name) %>%
  mutate(width = rescale_to(column = value, value = 15)
         ,width_f = (rescale_to(column = value, value = 25) %>%
           floor_divide(5))+1
  ) %>%
...
driver = data.frame(og = unique(temp_sp_cor_dbl$name)) %>%
  mutate(label = case_when(str_detect(og, "AGF")~"Agr., Forrestry,<br>and Fish"
           ...
           paste0(., case_when(str_detect(og, "T_")~"<br>(tonnage)",
                               T~"<br>(value)"))) %>%
  mutate(label = case_when(og %in% c("Tonnage\n(total)", "Value\n(total)")~og
                           ,T~label)) %>%
  arrange(label)


leaflet_list =
  list(driver$og[c(11:12)]
       ,driver$label[c(11:12)]) %>%
  # map(~.x[7]) %>%
  future_pmap(~{
    library(sf)
    ...
    tmp = tmp %>%
      mutate(q_flag = value > quantile(value, .05)) %>%
      filter(q_flag) %>%
      arrange(value)
    ...
    plot = tmp %>%
      ...
      addPolylines(#color = ~binpal(value)
        color = "#3232CC"
        ,weight = ~sizeNumeric(values = value, baseSize = baseSize)
        ,opacity = 1
        ,label = ~str_glue("{NO} - {name} - {value} - {width_f}")
        ,labelOptions = labelOptions(noHide = F, textOnly = F)) %>%
      addLegendLine(values = tmp$value,
                    title = tags$div(HTML(.y)),
    ...
    htmlwidgets::saveWidget(plot, file = here("output/freight_corridor_double", paste0(str_remove(.x, "\\\n"), ".html")))

    webshot::webshot(here("output/freight_corridor_double", paste0(str_remove(.x, "\\\n"), ".html")),
            file = here("output/freight_corridor_double/pics", paste0(str_remove(.x, "\\\n"), ".png")),
            cliprect = "viewport")
...
driver$og[c(11:12)] %>%
  ...
  map(~{
    file = str_remove(.x, "\\\n") %>%
      paste0(., ".png") %>%
      here("output/freight_corridor_double/pics", .)

    file %>%
      image_read() %>%
      image_crop("787x575+60+5") %>%
      image_write(., file)
  })
