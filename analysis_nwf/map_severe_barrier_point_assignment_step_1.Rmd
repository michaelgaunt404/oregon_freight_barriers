...
subtitle: "Barrier Point Assigment Step 1 Map"
...
#content in this section should be removed if in production - ok for dev -->
```{r message=FALSE}
if (!exists("map_object")){
  here("code/script_barrier_point_assignment_map.r") %>%  
    source()
}
```
...
### Barrier Map (by score)
This map colors barrier links and markers by their total barrier score.

```{r }
map_object
```

### Barrier Point Assignment Data 

```{r}
point_assigned_barriers_map %>%  
  st_drop_geometry() %>%  
  arrange(desc(total_score)) %>% 
  select(!c(index_severe)) %>% 
  dt_common(pl = 10, dom = "Btipr")
